...
export class SubscriptionNotificationService {
   ...
   * @param daysBeforeExpiration Quantos dias antes enviar o alerta (padrão: 7 dias)
    ...
    try {
      ...
      const notificationId = await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: `Sua assinatura "${subscriptionName}" vence ${daysBeforeExpiration === 0 ? 'hoje' : `em ${daysBeforeExpiration} dias`}`,
          data: {
            subscriptionName,
